...
router.post('/', authenticate, async (req, res) => {
  try {
    const { product_id, productId: productIdAlt, quantity, name, price, image } = req.body;
    const resolvedProductId = product_id || productIdAlt;

    if (!resolvedProductId || !quantity) {
    ...
    if (existing.rows.length > 0) {
      result = await pool.query(
        ...
        [quantity, req.user.id, resolvedProductId]
      );
    } else {
      // Look up product details from database if not provided
      ...
      result = await pool.query(
        `INSERT INTO cart_items (id, user_id, product_id, quantity, name, price, image, created_at)
         ...
         RETURNING *`,
        [req.user.id, resolvedProductId, quantity, productName, productPrice, productImage]
      );
    }
